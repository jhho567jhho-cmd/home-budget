  ...
  return (
    <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg shadow-lg p-6 border border-slate-700">
      ...
      <div className="space-y-4">
        {/* Title */}
        <div>
          <label className="block text-sm font-medium text-white mb-2">כותרת</label>
          <input
            type="text"
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="מה בדעתך היום?"
            className="w-full px-3 py-2 border border-slate-600 rounded-lg focus:outline-none focus:border-blue-400 bg-slate-700 text-white placeholder-slate-400"
          />
        </div>

        {/* Mood Selector */}
        <div>
          <label className="block text-sm font-medium text-white mb-2">איך אתה מרגיש היום?</label>
        ...
        {/* Content */}
        <div>
          <label className="block text-sm font-medium text-white mb-2">הרשומה שלי</label>
          <textarea
            value={content}
            onChange={(e) => setContent(e.target.value)}
            placeholder="כתוב כאן את המחשבות והרגשות שלך..."
            rows={6}
            className="w-full px-3 py-2 border border-slate-600 rounded-lg focus:outline-none focus:border-blue-400 resize-none bg-slate-700 text-white placeholder-slate-400"
          />
        </div>
